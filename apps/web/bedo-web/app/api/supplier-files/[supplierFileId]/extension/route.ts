...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    const result = await frappeCall<{ trainer_item: string }>("bedo_platform.api.web.request_supplier_extension", { supplier_file: supplierFile, payload }, session.user);
    ...
    return apiErrorResponse(error, "Supplier extension could not be requested.");
  }
}
