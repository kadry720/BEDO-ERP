...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    await frappeCall("bedo_platform.api.web.submit_case3_handover_confirmation", { trainer_item: trainerItem, payload }, session.user);
    const data = await frappeCall("bedo_platform.api.web.get_trainer_item_workspace", { trainer_item: trainerItem }, session.user);
    return NextResponse.json(data);
  } catch (error) {
    return apiErrorResponse(error, "Handover confirmation could not be submitted.");
  }
}
