...
export default async function Page({ params }: { params: Promise<{ parts?: string[] }> }) {
  const { session, freshSession } = await requireProjectScope("command-center");
  ...
  return <ProjectDetail project={project.project} initialItems={items} mode="command-center" />;
}
