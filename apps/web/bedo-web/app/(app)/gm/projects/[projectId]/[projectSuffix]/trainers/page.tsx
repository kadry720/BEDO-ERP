...
export default async function Page({ params }: { params: Promise<{ projectId: string; projectSuffix: string }> }) {
  const { session } = await requireProjectScope("gm");
  const { projectId, projectSuffix } = await params;
  const projectName = routeProjectName(projectId, projectSuffix);
